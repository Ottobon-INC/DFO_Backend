...
export class PatientsController {
    ...
    async create(@Body() body: any) {
        ...
        try {
            ...
            const { data: existing, error: existingError } = await supabase
                .from('sakhi_clinic_patients').select('id').eq('mobile', mobile).maybeSingle();
            ...
            const payload = this.utils.sanitizePayload({
                uhid, lead_id: tv(body.lead_id), name, relation: tv(body.relation), marital_status, gender,
    ...
    async getById(@Param('id') id: string) {
        ...
        const supabase = this.supabaseService.getClient();
        try {
            const { data, error } = await supabase.from('sakhi_clinic_patients').select('*').eq('id', id).single();
        ...
        try {
            ...
            if (mobile) {
                const { data: conflict, error: conflictError } = await supabase
                    .from('sakhi_clinic_patients').select('id').eq('mobile', mobile).neq('id', id).maybeSingle();
            ...
            const { data, error } = await supabase.from('sakhi_clinic_patients').update(sanitized).eq('id', id).select().single();
    ...
    async getPatientAppointments(@Param('id') id: string) {
        const supabase = this.supabaseService.getClient();
        try {
            const { data, error } = await supabase.from('sakhi_clinic_appointments').select('*').eq('patient_id', id).order('appointment_date', { ascending: false });
            if (error) throw error;
            return { success: true, data };
        } catch (error: any) {
    ...
    async getClinicalNotes(@Param('id') id: string) {
        ...
        const supabase = this.supabaseService.getClient();
        try {
            const { data, error } = await supabase.from('sakhi_clinic_patient_notes').select('id, patient_id, doctor_id, note, created_at').eq('patient_id', id).order('created_at', { ascending: false });
            if (error) throw error;
    ...
    async createClinicalNote(@Param('id') id: string, @Body() body: any) {
        ...
        try {
            ...
            const payload = this.utils.sanitizePayload({ patient_id: id, doctor_id: this.utils.isUuid(body?.doctor_id) ? body.doctor_id : null, note: body.note });
    ...
    async getNotes(@Param('id') id: string) {
        const supabase = this.supabaseService.getClient();
        try {
            const { data, error } = await supabase.from('sakhi_clinical_notes').select('*').eq('patient_id', id).order('created_at', { ascending: false });
            if (error) throw error;
            return { success: true, data };
        } catch (error: any) {
    ...
    async createNote(@Param('id') id: string, @Body() body: any) {
        ...
        try {
            ...
            const payload = this.utils.sanitizePayload({ patient_id: id, doctor_id: body.doctor_id, appointment_id: body.appointment_id, subjective: body.subjective, objective: body.objective, assessment: body.assessment, plan: body.plan });
    ...
    async getDocuments(@Param('id') id: string) {
        ...
        const supabase = this.supabaseService.getClient();
        try {
            const { data, error } = await supabase.from('sakhi_clinic_documents').select('*').eq('patient_id', id).order('uploaded_at', { ascending: false });
            if (error) throw error;
            return { success: true, data: data ?? [] };
        } catch (error: any) {
    ...
    async createDocument(@Param('id') id: string, @Body() body: any) {
        ...
        try {
            ...
            const { data, error } = await supabase.from('sakhi_clinic_documents')
                .insert({ patient_id: id, name, document_type, url: fileUrl, uploaded_at: new Date().toISOString() })
                .select().single();
            if (error) throw error;
            return { success: true, data };
        } catch (error: any) {
